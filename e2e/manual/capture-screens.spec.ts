...
async function pageLooksOk(page: Page): Promise<boolean> {
  const url = page.url();
  ...
  const status = await page.evaluate(() => {
    ...
    if (title.includes('404') || body.includes('page not found')) {
      return 404;
    }
...
async function shoot(page: Page, shot: Shot): Promise<void> {
  ...
  if (!(await pageLooksOk(page))) {
    test.info().annotations.push({ type: 'skip-shot', description: `${shot.name} skipped` });
  ...
  if (shot.name === 'dashboard') {
    ...
    if (await search.isVisible().catch(() => false)) {
      await search.fill('Symfony Beacon');
