...
| Concern | Rule |
|---|---|
| label | Never (FormKit); Twig captions in `messages` remain optional chrome |
...
| required | Always `false`, except **`per_page`** (`required: true`). Default forced in `AbstractGetFilterType::mergeFieldOptions()` unless the field passes an explicit `required`. FormKit YAML `field_types` **cannot** declare `required` (Configuration rejects it). |
