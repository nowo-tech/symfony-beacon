...
- `106-ops-ingest-hardening` — batched retention, quota cache, shared outbound URL policy (`nowo-tech/outbound-url-guard-bundle`, Mercure + webhooks). `allowPrivateUrls` still MUST NOT open cloud metadata.

## Amendment (EVENT-STORAGE + SSRF helper, 2026-08-25 / `106`)

Retention schedule in PRODUCTION.md links [docs/ops/EVENT-STORAGE.md](../../docs/ops/EVENT-STORAGE.md) (retention-first; no default partitioning). Outbound private-URL policy and Mercure hub validation share `nowo-tech/outbound-url-guard-bundle`; metadata stays blocked when private URLs are allowed.
