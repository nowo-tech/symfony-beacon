...
5. **Given** `exception.values[].stacktrace.frames`, **When** I open event detail, **Then** frames are listed; raw JSON remains available.
...
**Independent Test**: Configure `nowo_beacon.send.*` flags; capture an exception; assert omitted categories are absent from the Envelope payload and enabled ones are present.
...
- **FR-006**: Beacon event detail UI MUST present structured context (time, environment, release, runtime, framework, user, stack) plus raw payload.
...
- **SC-001**: An integrator can disable any single send category via YAML without code changes.
- **SC-002**: Event detail shows precise time and versions for a default-configured client event.
- **SC-003**: Unit/functional tests cover send flag omission and microsecond ingest parsing.
