...
- **Q4 (UI placement)**: Danger zone section on the project show page.
...
**Independent Test**: Seed issues/events for a project; open danger zone; confirm clear in modal; assert telemetry is gone and project/keys remain.

**Acceptance Scenarios**:

1. **Given** I am owner/admin on a project with history, **When** I open Clear history and confirm in the modal, **Then** issues/events/perf/stats for that project are removed and I stay on the project page with a success flash.
2. **Given** I am a member (not admin), **When** I view the project, **Then** I do not see Clear history (or the action is denied).
...
1. **Given** I am the owner, **When** I open Delete project, **Then** a modal warns that deletion is permanent and asks me to type the project name.
...
4. **Given** I am admin or member (not owner), **When** I view the project, **Then** Delete project is not available (or denied).

## Requirements *(mandatory)*

- **FR-001**: Danger zone on project show with Clear history and Delete project.
- **FR-002**: Both actions open warning modals (native `<dialog>` or equivalent accessible overlay).
- **FR-003**: Delete requires CSRF + exact project name confirmation server-side.
- **FR-004**: Clear requires CSRF + explicit confirm; removes history only.
