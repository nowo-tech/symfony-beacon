...
- **FR-005**: Guest shell / locale catalogues under `translations/messages.{locale}.yaml` for every enabled locale keep key parity with English (including `setup.token.*`). Docs in INSTALL.md and ADDING-LOCALES.md.
- **FR-006**: When platform catalogs are empty, `PlatformCatalogsSetupRedirectSubscriber` MUST redirect safe HTML GETs to SiteBackup `SetupPathPrefixResolver::resolve()` (locale-aware; after SiteBackup’s own gate). Authenticated non-admins are not redirected. Redirect Location MUST NOT embed `SITE_SETUP_TOKEN`.
