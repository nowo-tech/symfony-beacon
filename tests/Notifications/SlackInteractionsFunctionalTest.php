...
final class SlackInteractionsFunctionalTest extends DatabaseWebTestCase
{
    ...
    {
        [$client, $owner, $project] = $this->bootWithDemoProject('slack-resolve@example.com');
        $em = self::getContainer()->get(EntityManagerInterface::class);
        $owner->setSlackUserId('U_RESOLVE_OWNER');
        $em->flush();
    ...
    public function testResolveRejectedWithoutMappedSlackUser(): void
    {
        [$client, , $project] = $this->bootWithDemoProject('slack-resolve-anon@example.com');
        $em = self::getContainer()->get(EntityManagerInterface::class);
        ...
        $destination->setLabel('Ops Slack');
        $destination->setType(NotificationDestinationType::Slack);
        $destination->setEndpointUrl('https://hooks.slack.com/services/T/B/XR');
        $destination->setSigningSecret('slack-signing-secret');
        $destination->setEnabled(true);
        $destination->setCategories(['error']);
        ...
        $issue = new Issue();
        $issue->setProject($project);
        $issue->setFingerprint(hash('sha256', 'slack-resolve-anon'));
        $issue->setTitle('Slack resolve anon');
        $issue->setCulprit('demo');
        $issue->setLevel('error');
        $issue->setFirstSeen(new DateTimeImmutable());
        $issue->setLastSeen(new DateTimeImmutable());
        ...
        $this->postSlackAction($client, $destination, $project->getUuid(), $issue->getUuid(), 'resolve');
