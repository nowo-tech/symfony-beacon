...
- Misconfiguration of operator secrets (`.env`, Halite keys under `var/secrets/`, API keys) is an operational risk; see [PRODUCTION.md](docs/PRODUCTION.md) and [LEGAL-AND-COOKIES.md](docs/product/LEGAL-AND-COOKIES.md).
...
| Mercure hub URL (`095`) | Administration → Mercure rejects private IP literals, `localhost` / `.local` / `.internal`, and cloud metadata (`MercureHubUrlGuard` via `nowo-tech/outbound-url-guard-bundle`, same policy as webhooks). Docker Compose **service hostnames** (`mercure`, `php`) stay allowed without DNS resolve. Opt-in Ops `allowPrivateUrls` permits LAN literals. Metadata stays blocked either way: `169.254.0.0/16`, `fe80::/10`, Alibaba `100.100.100.200`, IPv4-mapped and decimal/hex forms, plus hosts `metadata` and `metadata.google.internal`. |
| Notification webhooks | Same guard as Mercure, plus DNS pin and `max_redirects: 0`. Keep **Allow private notification URLs** off in production. |
