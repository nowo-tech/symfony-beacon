...
describe('human-key-label controller', () => {
  ...
  it('no-ops when adjective or noun lists are empty', async () => {
    application.stop();
    document.body.innerHTML = `
      <div
        data-controller="human-key-label"
        data-human-key-label-adjectives-value='[]'
        data-human-key-label-nouns-value='["beacon"]'
      >
        <input data-human-key-label-target="label" type="text" value="keep" />
      </div>
    `;
    application = Application.start();
    application.register('human-key-label', HumanKeyLabelController);
