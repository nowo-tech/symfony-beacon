...
is_isolation_key() {
  case "$1" in
    COMPOSE_PROJECT_NAME|HTTP_PORT|HTTPS_PORT|HTTP3_PORT|DEFAULT_URI|MYSQL_DATABASE|VITE_PORT|REDIS_URL|MESSENGER_TRANSPORT_DSN|BEACON_DSN|DATABASE_URL|DATABASE_URL_RO|FRANKENPHP_MODE|FRANKENPHP_LOOP_MAX|FRANKENPHP_RESET_KERNEL|FRANKENPHP_WORKER_NUM|FRANKENPHP_WORKER_CONFIG)
      return 0
      ;;
  esac
  return 1
...
# FrankenPHP HTTP worker contract for isolated E2E (not Messenger). Make can override.
E2E_FRANKENPHP_MODE="${E2E_FRANKENPHP_MODE:-worker}"
E2E_FRANKENPHP_WORKER_NUM="${E2E_FRANKENPHP_WORKER_NUM:-4}"
E2E_FRANKENPHP_RESET_KERNEL="${E2E_FRANKENPHP_RESET_KERNEL:-false}"
upsert FRANKENPHP_MODE "$E2E_FRANKENPHP_MODE" "$OUT"
upsert FRANKENPHP_WORKER_NUM "$E2E_FRANKENPHP_WORKER_NUM" "$OUT"
upsert FRANKENPHP_RESET_KERNEL "$E2E_FRANKENPHP_RESET_KERNEL" "$OUT"
