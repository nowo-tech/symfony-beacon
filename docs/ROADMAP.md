...
| 6.49 | **SiteBackup 1.11 + durable setup-done**: pin bundle **1.11.0**; `progress_step_rows: true` (runtime DDL journal); `SetupDbDoneGuard` / redirect from `instance_settings.setup_completed_at` | Beacon | `056-setup-wizard` | **Done** (Unreleased after v1.14.0) |
...
| **Unreleased** | SiteBackup 1.11 per-step journal + DB-durable `/setup` done gate (`056` / 6.49); Redis dual-mode + payload promotions + access split (`099` / 6.50) |
...
Last updated: 2026-08-15 (**6.50** Unreleased — Redis horizontal scale + payload promotions + Identity/access thin; prior **6.49** SiteBackup 1.11; cut **v1.14.0**).
