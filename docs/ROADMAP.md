...
| 6.62 | **E2E cold-start circuit**: disposable `app_e2e_cold` / `:9461`; wipe → SiteBackup `/setup` API → wizard admin login; covers SETUP-01/07 + AUTH-10; CI job `e2e-cold` | Beacon | `110-e2e-cold-start-circuit` | **Done** (main) |
...
| **Unreleased** | Cold-start E2E (`110` / 6.62): disposable `app_e2e_cold` / `:9461`; wipe → `/setup` API → wizard admin login; CI `e2e-cold` |
...
Last updated: 2026-09-10 (Phase 6.62 cold-start E2E `110`; `108` shipped v1.26.0; retroactive `109` preload for v1.25.0 / Phase 6.61).
