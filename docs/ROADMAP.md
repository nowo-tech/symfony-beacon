...
| 6.66 | **Visual identity book + error manual docs**: `docs/identity/`; manual `08-errors.md` + captures; transparent PNG runtime art (amend `063`); CSP `<style>` nonces; html-twig smoke; `make test-e2e-mailpit`; SiteBackup panel_login Twig; **capture hygiene** (dashboard demo filter, MM schedule clear, cold setup EN, admin Groups demo row, EN legal footer) | Beacon | `114-identity-error-docs` | **Done** (v1.29.0) |
...
| **v1.30.0** | Operator legal editor (`115` / 6.67) at `/admin/legal`; Redis on `/health/ready`; quota `INCR`; `nowo-tech/outbound-url-guard-bundle` **1.0.0**; CKEditor **1.4.7** `html_sanitizer: strict`; tall create dialogs `--xl` |
| **Unreleased** | — |
...
Last updated: 2026-09-18 (v1.30.0 Phase 6.67 / `115` legal editor + Redis readiness; v1.29.3 generic legal seed; v1.29.0 Phase 6.66 / `114`).
