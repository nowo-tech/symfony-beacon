...
| 6.4g | **Self Beacon client** (dogfood): Packagist `beacon-bundle`; `make ready` / `make dogfood` → `BEACON_DSN` (auto `reload-env` when stale, v1.24.3); `make beacon-test` + `make beacon-suite` (incl. DB/long-content kinds) | Beacon | `058-self-beacon-client` | **Done** (v1.22.0 suite; DX/kinds v1.24.3) |
...
| 6.51 | **Phone input kit on Account → Profile**: `nowo-tech/phone-input-bundle` 1.2.1; CSP Stimulus prefix picker; `qr_login` **enabled** in base / **disabled** in `prod/` (AUTH-005); `.env.local` (REQ-ENV-003); `make restart` reloads env | Beacon | `100-phone-input-profile` | **Done** (v1.16.0) |
...
| 6.63 | **Product UI manual**: English `docs/manual/` screen catalog (purpose → contribution → screenshot) + fixed 1440×900 Playwright captures (warm inventory + cold setup); theme/locale demos once public + private; wiki Home index; `make docs-manual-screenshots{,-setup}` | Beacon | `111-product-ui-manual` | **Done** (v1.28.0; PR #53; prose polish 2026-09-11) |
| 6.64 | **Admin / alerts create modals**: New group, New project (admin), and New threshold rule as confirm-dialog modals (`?new=1` / `?new_threshold=1`); FormKit catalogue keys; manual capture paths | Beacon | `112-admin-create-modals` | **Done** (v1.28.0) |
| 6.65 | **Product E2E CI sharding & suite stabilization**: 4× Playwright `--shard=N/4` jobs + gate; `PLAYWRIGHT_WORKER_SUITE` gate; CI Mailer DSN; view-as-member CSRF exit; BreadcrumbKit full-page ephemeral writes | Beacon | `113-e2e-product-sharding` | **Done** (v1.28.2) |
| 6.66 | **Visual identity book + error manual docs**: `docs/identity/`; manual `08-errors.md` + captures; transparent PNG runtime art (amend `063`); CSP `<style>` nonces; html-twig smoke; `make test-e2e-mailpit`; SiteBackup panel_login Twig; **capture hygiene** (dashboard demo filter, MM schedule clear, cold setup EN, admin Groups demo row, EN legal footer) | Beacon | `114-identity-error-docs` | **Unreleased** |
...
| **v1.24.4** | Kit-over-shim (OTHER pass 40): HotReload **1.5.2**, LoginThrottle **3.2.0**, CookieConsent **1.9.7**, Pwa **1.5.0** `web_push`; host throttle/PWA/SW shims removed |
| **v1.24.5** | AUTH-005 / QR docs: base `qr_login.mode: enabled`; prod overlay `disabled` (drop stale `when@dev`/`when@test` narrative) |
...
| **Unreleased** | Visual identity book + error manual docs (`114` / 6.66): `docs/identity/`; `08-errors.md`; transparent runtime art; CSP style nonces; html-twig smoke; `make test-e2e-mailpit`; manual capture hygiene (incl. EN legal footer + admin Groups) |
...
Last updated: 2026-09-17 (Unreleased Phase 6.66 / `114` identity + error manual docs + capture hygiene + footer/admin Groups polish; v1.28.2 Phase 6.65 / `113` product E2E sharding + harness; v1.28.1 create-modal helpers; v1.28.0 Phase 6.63 / `111` + 6.64 / `112`).
