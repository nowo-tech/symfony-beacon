...
| 6.4g | **Self Beacon client** (dogfood): Packagist `beacon-bundle`; `make ready` / `make dogfood` → `BEACON_DSN`; `make beacon-test` + `make beacon-suite` (multi-event UI probe, explicit client tags) | Beacon | `058-self-beacon-client` | **Done** |
...
| 6.52 | **Kit CSP upstream + shared helpers**: PhoneInput **1.3.0**, CookieConsent **1.9.0**, FormKit **2.4.0**, UiKit **1.8.0**; drop host phone/cookie SCSS forks; CSRF/GET factories in FormKit; Stimulus peers from UiKit | Beacon | `101-kit-csp-shared-helpers` | **Done** (v1.17.0) |
...
Last updated: 2026-08-17 (isolated E2E / Morphicons in **v1.21.0**).
