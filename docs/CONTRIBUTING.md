...
16. **Symfony forms (host)** — every host Form Type in `src/**/Form/` MUST extend `Nowo\FormKitBundle\Form\FormKitAbstractType` (use `withBuilder` / `add*Field` / `mergeFieldOptions`; call `parent::__construct` when adding a custom constructor). Paint fields with `{% include 'form/_fields.html.twig' %}` (Type order; skip already rendered). Put `attr` / `row_attr` / labels on the Form Type. After custom layout `form_row`/`form_widget` calls, still include the loop before submit actions (`specs/077-form-type-field-loop/`).
17. **Shared Twig shells** — prefer includes over cloning chrome: confirm modals → `shared/_confirm_dialog.html.twig`; admin users/groups/projects lists → `admin/_list_page.html.twig`; dashboard feed panels → `dashboard/_feed_layout.html.twig` (`specs/086-dry-refactor/`). Pass optional vars with `|default` for `strict_variables`. **Form-bearing** confirm dialogs MUST set `header_wrapper: true` and `content_wrapper: true` (or paint `confirm-dialog__header` / `__content` / `__actions` explicitly). Prefer shell `cancel_label` / `submit_label` / `submit_disabled` over a custom `{% block actions %}`. Body-only confirms may stay flat. For `open_on_connect`, pass a boolean (or omit); never emit an empty `data-confirm-dialog-open-on-connect-value` (Stimulus Boolean treats empty as true — `086` FR-003c). Modal scrims MUST stay black (not `--color-ink`); rebuild assets after SCSS backdrop changes (`086` FR-003d).
...
19. **Password toggle** — layout lives in `_components.scss` (`.input-group.form-password-toggle`: flex, eye on the right, `gap: 0.5rem`). Keep `nowo_password_toggle.button_classes` including `input-group-text`; do not put Tailwind `flex`/`gap-*` on `toggle_container_classes` (they fight host SCSS). Password-manager extensions (NordPass, etc.) may inject icons on focus — that is not a Beacon layout bug.
20. **Compose / Make** — `make ensure-up` starts the stack with `docker compose up -d` when `php` does not respond to `exec` (no `--build`, no Vite). Exec-based targets (`test`, `phpstan`, `shell`, `console`, …) depend on it. Use `make up` for first boot / rebuild + asset build.
