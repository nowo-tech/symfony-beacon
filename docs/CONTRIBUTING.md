...
16. **Symfony forms (host + kit forks)** — do not hand-roll HTML forms in Twig; use `form_start` / `form_end` with Form Types. Every host Form Type in `src/**/Form/` MUST extend `Nowo\FormKitBundle\Form\FormKitAbstractType` (use `withBuilder` / `add*Field` / `mergeFieldOptions`; call `parent::__construct` when adding a custom constructor). CSRF-only POSTs may use `App\Shared\Form\CsrfOnlyType` (+ optional `CsrfOnlyFormFactory` / `csrf_action_form()`), and GET search forms should use `search_query_form()`. Paint fields with `{% include 'form/_fields.html.twig' %}` (Type order; skip already rendered). Put `attr` / `row_attr` / labels on the Form Type. After custom layout `form_row`/`form_widget` calls, still include the loop before submit actions (`specs/077-form-type-field-loop/`). Kit forks under `templates/bundles/` MUST follow the same rule (`REQ-TWIG-005`). AuthKit magic-login confirm uses Form CSRF via `MagicLoginConfirmType` (AuthKit ≥ 1.17).
