...
**Thin SCSS theme bridges (not Twig forks):** Cookie Consent `_cookie_consent.scss` (footer/position, `103`); PhoneInput `_phone_input.scss` imported from `app.scss` (kit Bootstrap `--bs-*` → Beacon `--color-*` / `data-theme`, including the portaled prefix dropdown). Do not re-fork kit picker JS or widget Twig. FrankenPHP hot reload is `nowo-tech/hot-reload-bundle` ≥**1.5.2** (`shared_worker`, CSP nonce/augment, `nowo:hot-reload:check`) — no host Vite/Twig Idiomorph client. See [ops/FRANKENPHP-HOT-RELOAD.md](ops/FRANKENPHP-HOT-RELOAD.md).
