...
| `GET /health/ready` | Readiness (database and Redis; Redis is skipped in `APP_ENV=test`). On failure, body uses a generic `error: unavailable` — no exception text (`050`). Messenger backlog is on authenticated `/metrics`, not this probe. |
...
Session auth via AuthKit (`/login`). Magic login (`/login/magic`) requires an encrypted instance Mailer DSN under **Administration → Mailer**. Share links grant time-limited viewer access (project-wide or issue-scoped). AuthKit **QR phone login** is **`enabled`** in the base `nowo_auth_kit` config (local / PHPUnit / E2E UC-AUTH-21/22); production sets `qr_login.mode: disabled` via `config/packages/prod/nowo_auth_kit.yaml` (AUTH-005) until SMS OTP. Approvers need `phoneVerifiedAt` (demo seed sets it for `admin@symfony-beacon.local`). Account → Profile phone uses `nowo-tech/phone-input-bundle` (country prefix → E.164).
