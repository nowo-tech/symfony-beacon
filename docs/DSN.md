...
Ingest **requires** `beacon_secret` (or the secret segment of the DSN) whenever the project API key has a secret stored (all keys generated by Beacon include one). Public-key-only auth is rejected with HTTP 403.
...
- `X-Beacon-Auth` header with `beacon_key` / **required** `beacon_secret` when the key has a secret
- Query string `?beacon_key=…&beacon_secret=…`
